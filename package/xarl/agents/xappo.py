...
XAPPO_EXTRA_OPTIONS = {
	...
	"buffer_options": {
		...
		'priority_can_be_negative': True, # Whether the priority can be negative. By default in DQN and DDPG it cannot while in PPO it can.
		...
		'prioritised_cluster_sampling_smoothness_degree': 2, # The higher it is, the more frequently sampled will be the clusters with the lowest priority. This is how many standard deviations to consider for estimating the upper maximum priority in the eta-normalisation of clusters priorities.
...
def xappo_postprocess_trajectory(policy, sample_batch, other_agent_batches=None, episode=None):
	...
	if policy.config["buffer_options"]["beta"] and 'weights' not in sample_batch:
