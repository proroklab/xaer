...
class ValueLoss(object):
	def __init__(self, global_step, loss, target, prediction, old_prediction=None):
		...
		if old_prediction != None:
			...
			print( "	Old Prediction shape: {}".format(self.old_prediction.get_shape()) )
		self.target = tf.stop_gradient(target)
		print( "	Target shape: {}".format(self.target.get_shape()) )
		print( "	Prediction shape: {}".format(self.prediction.get_shape()) )
		# Get reduce function
