...
CONFIG.update({
	...
	"buffer_options": {
		...
		'cluster_size': 2**14, # Default 50000. Maximum number of batches stored in a cluster (which number depends on the clustering scheme) of the experience buffer. Every batch has size 'replay_sequence_length' (default is 1).
	...
	"cluster_overview_size": 2**6, # cluster_overview_size <= train_batch_size -- When building a single train batch, do not sample a new cluster before x batches are sampled out of it. The closer is to train_batch_size, the faster is the algorithm.
})
